...
// FIX (item #7 — logo not used anywhere in the dashboard):
// Drop your real logo file at src/assets/logo.png (or .svg) and this
// will render it automatically. If the file is missing, it falls back
// to the ShieldIcon placeholder so the app never crashes on a missing
// asset — but you should add the real file so this fallback never shows.
let logoSrc = null;
try {
...
const NAV_SECTIONS = [
    ...
    },
    {
        ...
        items: [
            ...
            { label: 'Roster', path: '/roster', icon: <EventNoteIcon /> },
...
export default function Sidebar({ onOpenScan }) {
    ...
    return (
        <Box sx={{
            ...
            {/* Logo — uses src/assets/logo.png if present, else fallback icon */}
            <Box sx={{ mb: 4, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Box sx={{
                    width: 40, height: 40,
                    background: logoSrc ? 'transparent' : 'linear-gradient(135deg, #6366f1, #0d9488)',
                    borderRadius: '12px',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    boxShadow: logoSrc ? 'none' : '0 0 20px rgba(99, 102, 241, 0.3)',
                    overflow: 'hidden',
                }}>
                    {logoSrc ? (
                        <img src={logoSrc} alt="Hospital logo" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                    ) : (
