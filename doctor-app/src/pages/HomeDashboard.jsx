...
import { Typography, Grid, Card, CardContent, Box, Button, Chip, Avatar, Select, MenuItem, Stack } from '@mui/material';
...
export default function HomeDashboard({ onOpenScan }) {
    ...
    const [breakType, setBreakType] = React.useState('bio_break');
    ...
    // FIX (item #6 — "check it's getting data up to date and time"):
    // There was no clock or "last updated" indicator anywhere on this
    // dashboard, so there was no way to tell if the data on screen was
    // current or stale. Added a live clock plus a "data as of HH:MM:SS"
    // timestamp that updates every time the dashboard refetches.
    ...
    const handleEmergency = async () => {
        setIsEmergency(true);
        try {
            // FIX: was a raw fetch() with manually duplicated auth headers
            // (item #5 cleanup) — now goes through the shared apiClient,
            // which already attaches the Authorization header.
            await apiClient.post('/doctor/emergency/broadcast', {});
    ...
    const fetchData = React.useCallback(async (signal) => {
        try {
            const [queueData, profileData, statsData] = await Promise.all([
                clinicalService.getActiveQueue(signal),
    ...
    React.useEffect(() => {
        ...
        // Auto-refresh every 60s so the dashboard never goes stale while
        // a doctor leaves it open — directly addresses item #6.
    ...
    return (
        <Box sx={{ width: '100%', px: { xs: 2, md: 4 }, pt: 4, pb: 6 }}>
                    ...
                    {!isOnBreak ? (
                        <Stack direction="row" spacing={1} alignItems="center">
                            <Select
                                ...
                                sx={{
                                    minWidth: 140, color: '#fff', bgcolor: 'rgba(255,255,255,0.04)',
            ...
            {/* Live clock + data freshness indicator (item #6) */}
            <Box sx={{
                mb: 4, display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap',
                color: '#cbd5e1', fontSize: '0.85rem',
            }}>
                <AccessTimeIcon sx={{ fontSize: 16, color: '#0d9488' }} />
                <Typography variant="body2" sx={{ color: '#cbd5e1', fontWeight: 600 }}>
                    {dateStr} · {timeStr}
                </Typography>
                <Box sx={{ width: 4, height: 4, borderRadius: '50%', bgcolor: '#cbd5e1', opacity: 0.4 }} />
                <Typography variant="body2" sx={{ color: isLoading ? '#f59e0b' : '#10b981', fontWeight: 700 }}>
                    {isLoading ? 'Syncing data…' : `Data as of ${lastUpdatedStr}`}
                </Typography>
            </Box>
                        ...
                        <Box sx={{ p: 2 }}>
                            {isLoading ? (
                                <Box sx={{ p: 8, textAlign: 'center' }}>
                                    <Typography sx={{ color: '#cbd5e1', fontFamily: 'Space Mono', fontWeight: 700 }}>LOADING HIGH-FIDELITY DATA...</Typography>
                                </Box>
                            ) : !patients || patients.length === 0 ? (
                                <Box sx={{ p: 8, textAlign: 'center' }}>
                                    <Typography sx={{ color: '#cbd5e1', fontWeight: 600 }}>No active clinical encounters recorded today.</Typography>
                                </Box>
                            ) : (
                        ...
                        <Card elevation={0} sx={{ background: 'rgba(255,255,255,0.01)', backdropFilter: 'blur(30px)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '24px' }}>
                            ...
                            <Box sx={{ p: 4 }}>
                                <Grid container rowSpacing={4} columnSpacing={2}>
                                    ...
                                    <Grid item xs={6}><MiniStat label="Queue" value={queueStarted ? 'Active' : 'Idle'} color={queueStarted ? '#10b981' : '#cbd5e1'} /></Grid>
                                </Grid>
                            </Box>
                        </Card>
                    </Box>
...
const StatCard = ({ title, value, change, color, icon }) => (
    <Card className="glass-card" elevation={0} sx={{ height: '100%', position: 'relative', overflow: 'hidden', transition: 'all 0.4s cubic-bezier(0.4, 0, 0.2, 1)', background: 'rgba(255, 255, 255, 0.02)', '&:hover': { transform: 'translateY(-8px)', background: 'rgba(255, 255, 255, 0.05)', boxShadow: `0 20px 40px -15px rgba(0, 0, 0, 0.8), 0 0 20px ${color}22`, '& .stat-icon': { transform: 'scale(1.2) rotate(-5deg)', opacity: 0.5 } } }}>
        ...
        <CardContent sx={{ p: 4 }}>
            ...
            <Typography variant="caption" sx={{ color: '#cbd5e1', mb: 1, display: 'block', fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase' }}>{title}</Typography>
        ...
        <Box sx={{ p: 2.5, borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', transition: 'all 0.2s', '&:hover': { bgcolor: 'rgba(255,255,255,0.02)', cursor: 'pointer' }, '&:last-child': { borderBottom: 'none' } }} onClick={() => id && navigate(`/patient/${id}`)}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                <Box>
                    ...
                    <Typography variant="caption" sx={{ color: '#cbd5e1', fontWeight: 600 }}>{condition}</Typography>
                </Box>
            </Box>
...
const QuickActionButton = ({ label, icon, onClick, color }) => (
    <Button fullWidth className="glass-card" onClick={onClick} sx={{ display: 'flex', flexDirection: 'column', py: 3, border: '1px solid rgba(255,255,255,0.03)', color: '#fff', background: 'rgba(255, 255, 255, 0.02)', transition: 'all 0.3s', borderRadius: '18px', '&:hover': { background: 'rgba(255, 255, 255, 0.05)', transform: 'translateY(-4px)', boxShadow: `0 10px 30px rgba(0,0,0,0.5), 0 0 15px ${color}22`, borderColor: `${color}44` } }}>
        ...
        <Typography variant="caption" sx={{ textTransform: 'uppercase', fontWeight: 800, letterSpacing: '0.05em', color: '#cbd5e1' }}>{label}</Typography>
    </Button>
);
...
const MiniStat = ({ label, value, color = '#fff' }) => (
    <Box sx={{ textAlign: 'center' }}>
        ...
        <Typography variant="caption" sx={{ color: '#cbd5e1', fontWeight: 700, textTransform: 'uppercase', display: 'block', mt: 0.5 }}>{label}</Typography>
    </Box>
);
