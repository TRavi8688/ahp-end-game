...
const TYPE_CONFIG = {
    ...
    system: { icon: <NotificationsIcon />, color: '#cbd5e1', bg: 'rgba(255,255,255,0.05)' },
};
            ...
            <Box sx={{ mb: 6, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 3 }}>
                ...
                <Box sx={{ display: 'flex', gap: 2 }}>
                    <Tooltip title="Refresh">
                        <IconButton onClick={() => fetchNotifications(false)} sx={{ color: '#cbd5e1', bgcolor: 'rgba(255,255,255,0.05)', '&:hover': { color: '#fff' } }}>
                            <RefreshIcon />
                        </IconButton>
                    </Tooltip>
                    {unreadCount > 0 && (
                ...
                {[{ label: 'All', value: 'all' }, { label: `Unread (${unreadCount})`, value: 'unread' }].map(f => (
                    <Button key={f.value} onClick={() => setFilter(f.value)} size="small"
                        sx={{
                            ...
                            color: filter === f.value ? '#0d9488' : '#cbd5e1',
                            ...
                            <Box key={notif.id}
                                ...
                                <Box sx={{ flex: 1, minWidth: 0 }}>
                                    <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 2, mb: 0.5 }}>
                                        <Typography variant="subtitle2" sx={{ fontWeight: notif.is_read ? 600 : 900, color: notif.is_read ? '#cbd5e1' : '#fff' }}>
                                            {notif.title}
                                        </Typography>
                                        <Typography variant="caption" sx={{ color: '#cbd5e1', fontFamily: 'Space Mono', whiteSpace: 'nowrap', flexShrink: 0, fontWeight: 600 }}>
                                            {timeAgo(notif.created_at)}
                                        </Typography>
                                    </Box>
                                    <Typography variant="body2" sx={{ color: notif.is_read ? '#cbd5e1' : '#cbd5e1', lineHeight: 1.6 }}>
                                        {notif.body}
                                    </Typography>
                                </Box>
                                {/* Unread dot */}
