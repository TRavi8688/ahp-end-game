// FULL FIX (item #10 — leave requests silently vanishing):
//   clinicalService.markLeave/getLeaveHistory/cancelLeave were already
//   calling the right paths (/doctor/leave), but those endpoints never
//   existed on the backend until now (see backend patch). Combined with
//   the .env URL bug, every call failed silently, and this page's
//   fallback behavior masked it: on fetch failure it showed fake
//   MOCK_LEAVES, and on submit failure it just appended the new leave
//   to local state and told the doctor "submitted successfully" — that
//   request was NEVER saved anywhere and vanished on refresh. With the
//   backend now built and the API URL fixed, that masking is not just
//   unnecessary, it's actively dangerous (hides real failures from the
//   doctor). Removed entirely — failures are now shown as failures.
...
const LEAVE_TYPES = ['Day Off', 'Half Day', 'Emergency Leave', 'Conference / CME', 'Personal', 'Vacation', 'Sick'];
...
export default function LeaveManagement() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setFormError(
                ...
                'Could not submit leave request. Please check your connection and try again.'
            );
        } finally {
            setSubmitting(false);
        }
    ...
    const handleCancel = async (leaveId) => {
        ...
        try {
            ...
            showToast('Could not cancel this leave request. Please try again.', 'error');
        } finally {
        ...
        <Box sx={{ maxWidth: 1000, mx: 'auto', px: { xs: 2, md: 4 }, pt: 4, pb: 8 }}>
                    ...
                    <Grid item xs={12} sm={4} key={s.label}>
                        <Card elevation={0} sx={{ p: 3, bgcolor: s.bg, border: `1px solid ${s.border}`, borderRadius: '20px', display: 'flex', alignItems: 'center', gap: 3 }}>
                            ...
                            <Box>
                                ...
                                <Typography variant="caption" sx={{ color: '#cbd5e1', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1 }}>{s.label}</Typography>
                            </Box>
                        </Card>
                    </Grid>
                ))}
            ...
            <Card elevation={0} sx={{ bgcolor: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '24px', overflow: 'hidden' }}>
                ...
                {isLoading ? (
                    ...
                    <Box sx={{ p: 8, textAlign: 'center' }}>
                        <Typography variant="h6" sx={{ color: '#f59e0b', fontWeight: 700 }}>Couldn't load leave history</Typography>
                        <Typography variant="body2" sx={{ color: '#cbd5e1', mt: 1 }}>Check your connection and try refreshing the page.</Typography>
                    </Box>
                ) : leaves.length === 0 ? (
                    <Box sx={{ p: 8, textAlign: 'center' }}>
                        <EventBusyIcon sx={{ fontSize: 48, color: 'rgba(255,255,255,0.1)', mb: 2 }} />
                        <Typography variant="h6" sx={{ color: '#cbd5e1', fontWeight: 700 }}>No leave requests</Typography>
                        ...
                        return (
                            <Box key={leave.id} sx={{ p: 3, borderBottom: index < leaves.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none', display: 'flex', alignItems: 'center', gap: 3, flexWrap: 'wrap', '&:hover': { bgcolor: 'rgba(255,255,255,0.02)' } }}>
                                ...
                                <Box sx={{ flex: 1, minWidth: 200 }}>
                                    ...
                                    {leave.reason && (
                                        <Typography variant="caption" sx={{ color: '#cbd5e1', display: 'block', mt: 0.5 }}>{leave.reason}</Typography>
                                    )}
                                </Box>
            ...
            <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth
                ...
                <DialogContent>
                    ...
                    {/* Leave Type */}
                    <Typography variant="caption" sx={{ color: '#cbd5e1', fontWeight: 700, letterSpacing: 1, display: 'block', mb: 1, mt: 1 }}>LEAVE TYPE</Typography>
                        ...
                        {LEAVE_TYPES.map(t => (
                            <Chip key={t} label={t} onClick={() => setForm(f => ({ ...f, leave_type: t }))} clickable
                                sx={{
                                    ...
                                    color: form.leave_type === t ? '#0d9488' : '#cbd5e1',
                ...
                <DialogActions sx={{ px: 3, pb: 3, gap: 2 }}>
                    <Button onClick={() => setDialogOpen(false)} sx={{ color: '#cbd5e1', fontWeight: 700, borderRadius: '12px', px: 3 }}>Cancel</Button>
