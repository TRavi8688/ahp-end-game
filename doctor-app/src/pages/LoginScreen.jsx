...
function ForgotPasswordScreen({ onBack }) {
    ...
    return (
        <Box sx={{ width: '100%', maxWidth: 450 }}>
            <Box sx={{ mb: 4 }}>
                <IconButton onClick={onBack} sx={{ color: '#cbd5e1', mb: 2, ml: -1 }}>
                    <ArrowBackIcon />
            ...
            {step === 'request' && (
                <>
                    <TextField fullWidth label="Hospyn ID or Email" value={identifier} onChange={e => setIdentifier(e.target.value)} sx={inputSx} InputProps={{ startAdornment: <EmailIcon sx={{ mr: 2, color: '#cbd5e1' }} /> }} />
            ...
            {step === 'verify' && (
                <>
                    ...
                    <Button onClick={handleRequest} sx={{ color: '#cbd5e1', width: '100%', textTransform: 'none' }}>Resend code</Button>
                </>
            )}
            {step === 'reset' && (
                <>
                    <TextField fullWidth label="New Password" type={showNewPw ? 'text' : 'password'} value={newPassword} onChange={e => setNewPassword(e.target.value)} sx={inputSx}
                        InputProps={{ startAdornment: <LockIcon sx={{ mr: 2, color: '#cbd5e1' }} />, endAdornment: (<IconButton onClick={() => setShowNewPw(v => !v)} edge="end" sx={{ color: '#cbd5e1' }}>{showNewPw ? <VisibilityOffIcon /> : <VisibilityIcon />}</IconButton>) }} />
                    <TextField fullWidth label="Confirm New Password" type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} sx={{ ...inputSx, mb: 4 }} InputProps={{ startAdornment: <LockIcon sx={{ mr: 2, color: '#cbd5e1' }} /> }} />
            ...
            <Grid container sx={{ flex: 1, zIndex: 1 }}>
                ...
                <Grid item xs={12} md={6} sx={{ display: { xs: 'none', md: 'flex' }, flexDirection: 'column', justifyContent: 'center', p: 10 }}>
                    <Box sx={{ mb: 6 }}>
                        ...
                        <Typography variant="h6" sx={{ color: '#cbd5e1', fontWeight: 400, maxWidth: 500, lineHeight: 1.6, mb: 6 }}>
                        ...
                        <Box sx={{ p: 3, maxWidth: 450, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '16px' }}>
                            <Box sx={{ display: 'flex', gap: 2 }}>
                                <Box sx={{ flex: 1 }}>
                                    ...
                                    <Typography variant="body2" sx={{ color: '#cbd5e1', mt: 0.5 }}>End-to-end encrypted medical sessions.</Typography>
                                </Box>
                                ...
                                <Box sx={{ flex: 1 }}>
                                    ...
                                    <Typography variant="body2" sx={{ color: '#cbd5e1', mt: 0.5 }}>Sub-second clinical data sync.</Typography>
                                </Box>
                            </Box>
                        </Box>
                    </Box>
                    ...
                    <Box sx={{ width: '100%', maxWidth: 450, p: { xs: 4, md: 8 }, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '32px', backdropFilter: 'blur(24px)' }}>
                        {showForgotPassword ? (
                            ...
                            <>
                                ...
                                <Typography variant="body2" sx={{ color: '#cbd5e1', mb: 6 }}>Initialize your secure practitioner session.</Typography>

                                <Tabs value={loginMode} onChange={(e, v) => { setLoginMode(v); setOtpSent(false); setPasswordOrOtp(''); }}
                                    sx={{ mb: 4, minHeight: 48, borderRadius: 3, bgcolor: 'rgba(255,255,255,0.03)', p: 0.5, '& .MuiTabs-indicator': { display: 'none' }, '& .MuiTab-root': { textTransform: 'none', fontWeight: 800, color: '#cbd5e1', borderRadius: 2, '&.Mui-selected': { bgcolor: 'rgba(255,255,255,0.05)', color: 'white' } } }}
                                    variant="fullWidth">
                                ...
                                <form onSubmit={handleLogin}>
                                    ...
                                    <TextField fullWidth placeholder="Hospyn ID, Email or Phone" value={identifier} onChange={(e) => setIdentifier(e.target.value)} sx={{ mb: 3, ...inputSx }} InputProps={{ startAdornment: <SmartphoneIcon sx={{ mr: 2, color: '#cbd5e1' }} /> }} />

                                    {loginMode === 'otp' ? (
                                        !otpSent ? (
                                            <Box>
                                                <RadioGroup row value={otpChannel} onChange={(e) => setOtpChannel(e.target.value)} sx={{ mb: 3, justifyContent: 'center' }}>
                                                    <FormControlLabel value="sms" control={<Radio sx={{ color: '#cbd5e1', '&.Mui-checked': { color: '#6366f1' } }} />} label={<Typography sx={{ color: '#cbd5e1', fontSize: '0.85rem', fontWeight: 600 }}>SMS</Typography>} />
                                                    <FormControlLabel value="email" control={<Radio sx={{ color: '#cbd5e1', '&.Mui-checked': { color: '#6366f1' } }} />} label={<Typography sx={{ color: '#cbd5e1', fontSize: '0.85rem', fontWeight: 600 }}>Email</Typography>} />
                                                </RadioGroup>
                                        ...
                                        <>
                                            <TextField fullWidth type={showPassword ? 'text' : 'password'} placeholder="Secure Passkey" value={passwordOrOtp} onChange={(e) => setPasswordOrOtp(e.target.value)} sx={{ mb: 1, ...inputSx }}
                                                InputProps={{ startAdornment: <LockIcon sx={{ mr: 2, color: '#cbd5e1' }} />, endAdornment: (<IconButton onClick={() => setShowPassword(v => !v)} edge="end" sx={{ color: '#cbd5e1' }}>{showPassword ? <VisibilityOffIcon /> : <VisibilityIcon />}</IconButton>) }} />
                                    ...
                                    <Box sx={{ mt: 6, textAlign: 'center' }}>
                                        <Typography variant="caption" sx={{ color: '#cbd5e1', display: 'block', mb: 2 }}>Unauthorized access attempts are monitored and logged.</Typography>
