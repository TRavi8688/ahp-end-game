    ...
    <div className="bg-white rounded-2xl shadow-card p-4">
        ...
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            <p className="text-xs text-gray-400">
              {order.patient_code ? `${order.patient_code} · ` : ''}{order.patient_phone}
            </p>
          </div>
          {order.token_number ? (
            ...
            <span className="text-xs text-gray-400 flex items-center gap-1">
              ...
              {order.shared_at
                ? new Date(order.shared_at).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })
                : ''}
            </span>
          )}
        </div>
      ...
      {order.status === 'pending' && (
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className="bg-red-50 hover:bg-red-100 text-red-600 font-semibold py-2 rounded-xl text-sm flex items-center justify-center gap-1.5"
          >
            <X className="w-4 h-4" /> Reject
          </button>
          <button
            onClick={() => onAccept(order.id)}
            className="bg-success-50 hover:bg-success-100 text-success-700 font-semibold py-2 rounded-xl text-sm flex items-center justify-center gap-1.5"
          >
      ...
      {order.status === 'accepted' && (
        <button
          ...
          className="w-full bg-primary-50 hover:bg-primary-100 text-primary-700 font-semibold py-2 rounded-xl text-sm"
        >
      ...
      {order.status === 'preparing' && (
        <button
          ...
          className="w-full bg-primary-600 hover:bg-primary-700 text-white font-semibold py-2 rounded-xl text-sm"
        >
          Mark Ready
        </button>
      )}
      {order.status === 'ready' && (
        <button
          ...
          className="w-full bg-success-600 hover:bg-success-700 text-white font-semibold py-2 rounded-xl text-sm"
        >
          Delivered
        </button>
      )}
      {(order.status === 'delivered' || order.status === 'rejected') && (
        <span
          className={`inline-block text-xs font-semibold px-2.5 py-1 rounded-full ${
            order.status === 'delivered' ? 'bg-success-100 text-success-700' : 'bg-red-100 text-red-600'
          }`}
        >
        ...
        <div className="p-4 space-y-4">
          <div className="bg-lavender-50 rounded-2xl p-4">
            ...
            <p className="text-sm text-gray-500">
              {order.patient_phone} {order.patient_code && `· ${order.patient_code}`}
            </p>
          </div>
          {order.prescription_image_url ? (
            <img src={order.prescription_image_url} alt="Prescription" className="w-full rounded-xl" />
          ) : (
            <div className="bg-lavender-50 rounded-2xl p-6 text-center text-sm text-gray-400">
...
export default function Orders() {
  ...
  useEffect(() => {
    fetchOrders();
    const interval = setInterval(fetchOrders, 20000); // poll every 20s for new orders
  ...
  const act = async (id, action, body) => {
    try {
      ...
      console.error(err);
      showToast(err.response?.data?.detail || 'Action failed. Please try again.', 'error');
    }
  };
            ...
            {['today', 'week', 'month'].map((p) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-semibold capitalize ${
                  period === p ? 'bg-primary-100 text-primary-700' : 'bg-white text-gray-400 shadow-card'
                }`}
              >
                {p}
              </button>
