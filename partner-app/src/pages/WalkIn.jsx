...
import {
  Plus, User, Phone, Camera, Upload, SkipForward, Search, Trash2,
  Banknote, Smartphone, CreditCard, Check, Printer, Download, Share2,
  ChevronLeft, Receipt, AlertCircle,
} from 'lucide-react';
...
function CustomerStep({ onNext }) {
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 text-red-700 px-3 py-2 rounded-xl text-sm mb-4 flex items-center gap-2">
      ...
      <button
        ...
        disabled={saving}
        className="w-full bg-primary-600 hover:bg-primary-700 text-white font-semibold py-3.5 rounded-full mt-5 disabled:opacity-60"
      >
...
function PrescriptionUploadStep({ onNext, onBack }) {
  ...
  const handleContinue = async () => {
    // NOTE: /pharmacy/upload-prescription endpoint does not yet exist on the backend.
    // The CheckoutRequest schema also doesn't include prescription_image_url.
    // We store the file in memory for display only (shown in the success screen)
    // and pass null to the sale. When the backend adds the endpoint, swap this
    // for a real multipart POST.
    setUploading(false);
    onNext({ prescriptionUrl: null, prescriptionFile: file || null });
  };
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      {error && (
        <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 px-3 py-2 rounded-xl text-sm mb-3 flex items-center gap-2">
          <AlertCircle className="w-4 h-4 shrink-0" /> {error}
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl shadow-card p-6 text-center">
        {preview ? (
          <img src={preview} alt="Prescription" className="w-full rounded-xl mb-4 max-h-64 object-contain" />
        ) : (
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className="bg-primary-50 hover:bg-primary-100 text-primary-700 font-semibold py-2.5 rounded-xl text-sm flex items-center justify-center gap-1.5"
          >
      ...
      <button
        ...
        className="w-full bg-primary-600 hover:bg-primary-700 text-white font-semibold py-3.5 rounded-full mt-5 disabled:opacity-60"
      >
        {uploading ? 'Uploading…' : file ? 'Continue' : 'Continue'}
      </button>
      {!file && (
        <button
          onClick={() => onNext({ prescriptionUrl: null })}
          className="w-full text-gray-400 font-semibold py-2 mt-1 text-sm flex items-center justify-center gap-1"
        >
...
function MedicineEntryStep({ cart, setCart, onNext, onBack }) {
  ...
  useEffect(() => {
    ...
    setResults(
      allInventory.filter((i) => i.item_name.toLowerCase().includes(query.toLowerCase())).slice(0, 8)
    );
  }, [query, allInventory]);
  ...
  const addToCart = (item) => {
    setCart((prev) => {
      ...
      if (existing) {
        return prev.map((l) =>
          l.inventory_item_id === item.id ? { ...l, quantity: l.quantity + 1 } : l
        );
      }
      return [
        ...prev,
        { inventory_item_id: item.id, name: item.item_name, unit_price: item.unit_price, stock: item.stock_quantity, quantity: 1 },
      ];
    });
    setQuery('');
  };
  ...
  return (
    <div className="p-4 sm:p-6">
        ...
        <div className="bg-white rounded-2xl shadow-card mb-4 overflow-hidden">
          {results.map((item) => (
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3 hover:bg-lavender-50 border-b border-lavender-50 last:border-0"
            >
          ...
          {cart.map((line) => (
            <div key={line.inventory_item_id} className="bg-white rounded-xl shadow-card p-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                    ...
                    setCart((prev) =>
                      prev.map((l) =>
                        l.inventory_item_id === line.inventory_item_id
                          ? { ...l, quantity: parseInt(e.target.value, 10) || 1 }
                          : l
                      )
                    )
                  }
                ...
                />
                <button
                  onClick={() => setCart((prev) => prev.filter((l) => l.inventory_item_id !== line.inventory_item_id))}
      ...
      <button
        ...
        className="w-full bg-primary-600 hover:bg-primary-700 text-white font-semibold py-3.5 rounded-full disabled:opacity-50"
      >
        Review Bill
      </button>
    </div>
    ...
    <div className="p-4 sm:p-6">
      ...
      <div className="bg-white rounded-2xl shadow-card overflow-hidden mb-4">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-gray-400 text-xs border-b border-lavender-50">
      ...
      <button
        onClick={onNext}
        className="w-full bg-primary-600 hover:bg-primary-700 text-white font-semibold py-3.5 rounded-full"
      >
        Choose Payment
      </button>
    </div>
...
// ── Step 5: Payment ──────────────────────────────────────────────────────────
function PaymentStep({ cart, customerState, prescriptionState, onComplete, onBack }) {
  ...
  const handleComplete = async () => {
    ...
    setError('');
    try {
      // CheckoutRequest fields: walkin_customer_id | patient_id, items[], payment_method
      // prescription_image_url is NOT in the backend schema (endpoint pending)
      const res = await apiClient.post('/pharmacy/sales', {
        walkin_customer_id: customerState.walkin_customer_id || null,
        patient_id:         null,
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {methods.map((m) => (
          <button
            ...
            className={`bg-white rounded-2xl p-4 flex flex-col items-center gap-2 shadow-card border-2 ${
              method === m.key ? 'border-primary-500' : 'border-transparent'
            }`}
          >
            <m.icon className={`w-6 h-6 ${method === m.key ? 'text-primary-600' : 'text-gray-400'}`} />
            <span className={`text-xs font-bold ${method === m.key ? 'text-primary-700' : 'text-gray-500'}`}>
              {m.label}
            </span>
          </button>
        ))}
      ...
      <button
        ...
        disabled={submitting}
        className="w-full bg-success-600 hover:bg-success-700 text-white font-bold py-4 rounded-full shadow-floating disabled:opacity-60"
      >
...
function BillSuccessStep({ sale, onDone }) {
  const handleWhatsApp = () => {
    ...
    const itemsList = (sale.items || [])
      .map((i) => `  • ${i.medicine_name} × ${i.quantity} — ₹${i.line_total?.toFixed(2) || '0.00'}`)
      .join('\n');
  ...
  const handlePdf = () => {
    apiClient.get(`/pharmacy/sales/${sale.id}/pdf`, { responseType: 'blob' })
      .then((res) => {
  ...
  return (
    <div className="p-4 sm:p-6 text-center">
      ...
      <div className="bg-white rounded-2xl shadow-card p-4 mb-6 text-left">
        ...
        <div className="flex justify-between font-bold text-ink-900 pt-2 mt-1">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <button
          ...
          className="bg-white shadow-card rounded-xl py-3 flex flex-col items-center gap-1.5 text-xs font-semibold text-ink-900"
        >
...
export default function WalkIn() {
  ...
  if (stage === 'dashboard')    return <><WalkInDashboard onStart={() => setStage('customer')} /><Toast toast={toast} onDismiss={() => setToast(null)} /></>;
  if (stage === 'customer')     return <CustomerStep onNext={(s) => { setCustomerState(s); setStage('prescription'); }} />;
  if (stage === 'prescription') return <PrescriptionUploadStep onNext={(s) => { setPrescriptionState(s); setStage('medicines'); }} onBack={() => setStage('customer')} />;
  ...
  if (stage === 'payment')      return <PaymentStep cart={cart} customerState={customerState} prescriptionState={prescriptionState} onComplete={(sale) => { setCompletedSale(sale); setStage('success'); }} onBack={() => setStage('cart')} />;
