...
export const appointmentService = {
  ...
  searchDoctors: async (params = {}, signal) => {
    const response = await apiClient.get('/api/v1/healthcare/doctors/search', { params, signal });
  ...
  getDoctorSlots: async (doctorId, date, signal) => {
    const response = await apiClient.get(`/api/v1/healthcare/doctors/${doctorId}/slots`, {
      params: { date },
      signal,
    });
  ...
  bookAppointment: async (payload) => {
    const response = await apiClient.post('/api/v1/healthcare/appointments', payload);
  ...
  getAppointments: async (signal) => {
    const response = await apiClient.get('/api/v1/healthcare/appointments', { signal });
    return response.data;
  },
  ...
  getAppointmentDetail: async (appointmentId, signal) => {
    const response = await apiClient.get(`/api/v1/healthcare/appointments/${appointmentId}`, { signal });
    return response.data;
  },
  ...
  cancelAppointment: async (appointmentId, reason = '') => {
    const response = await apiClient.post(`/api/v1/healthcare/appointments/${appointmentId}/cancel`, { reason });
    return response.data;
  },
  ...
  getQueueStatus: async (token, signal) => {
    const response = await apiClient.get(`/api/v1/healthcare/doctor/queue/status/${token}`, { signal });
