...
export default function HealthIdScreen() {
    ...
    const qrValue = profile?.hospyn_id || 'HOSPAIN-PENDING';

    const copyToClipboard = async () => {
        if (!qrValue || qrValue === 'HOSPAIN-PENDING') return;
        await Clipboard.setStringAsync(qrValue);
        Alert.alert('Copied!', 'Your Hospain ID has been copied to your clipboard.');
    };
    ...
    const isLight = Theme.colors.primary === '#13396B';
    ...
    return (
        <ScrollView style={[styles.container, { backgroundColor: Theme.colors.background }]} contentContainerStyle={{ padding: 24, alignItems: 'center' }}>
            ...
            <Text style={[styles.subtitle, { color: Theme.colors.textMuted }]}>Scan this QR code at any Hospain-enabled facility for instant clinical history access.</Text>
            ...
            >
                <View style={styles.cardHeader}>
                    <View>
                        <Text style={[styles.brandName, { color: isLight ? '#13396B' : '#fff' }]}>HOSPAIN</Text>
