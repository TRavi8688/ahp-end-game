...
// FIXED: keepPreviousData (v4 API) → placeholderData: keepPreviousData (v5 API)
...
function LogRow({ log }) {
  return (
    <tr>
      <td>
        <div className="font-mono text-xs text-slate-300">
          {log.timestamp || log.created_at
            ? new Date(log.timestamp || log.created_at).toLocaleString('en-IN', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' })
            : '—'}
        </div>
      </td>
      <td>
      ...
      <td><span className={`badge ${actionColor(log.action)} text-xs`}>{log.action || '—'}</span></td>
      <td>
        <div className="text-xs text-slate-300">{log.resource_type || log.resource || '—'}</div>
        {log.resource_id && <div className="font-mono text-xs text-slate-600 truncate max-w-[120px]">{(log.resource_id || '').substring(0, 14)}</div>}
      </td>
      <td><div className="font-mono text-xs text-slate-500">{log.ip_address || log.ip || '—'}</div></td>
      <td><div className="text-xs text-slate-500 truncate max-w-[160px]">{log.details || log.metadata ? JSON.stringify(log.details || log.metadata).substring(0, 60) : '—'}</div></td>
    </tr>
  );
}
...
export default function AuditLogViewer() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    ...
    staleTime: 30 * 1000,
    // FIXED: v5 API uses placeholderData: keepPreviousData (not keepPreviousData: true)
  ...
  return (
    <div className="h-full flex flex-col animate-fadeIn">
      ...
      <div className="p-6 border-b border-white/5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative">
            ...
            <input className="input-dark pl-8 py-2 text-xs w-48" placeholder="Filter by user ID..."
              value={userId} onChange={e => { setUserId(e.target.value); handleFilterChange(); }} />
          </div>
          <select className="input-dark py-2 text-xs w-40" value={actionType}
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input type="date" className="input-dark pl-8 py-2 text-xs w-40" value={from}
                onChange={e => { setFrom(e.target.value); handleFilterChange(); }} />
            </div>
            <span className="text-slate-600 text-xs">to</span>
            <input type="date" className="input-dark py-2 text-xs w-36" value={to}
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading ? (
          <div className="flex items-center justify-center py-16"><Loader2 className="animate-spin text-violet-500" size={28} /></div>
        ) : isError ? (
          ...
          <table className="data-table">
            ...
            <tbody>
              {logs.map((log, idx) => <LogRow key={log.id || `${log.timestamp}-${idx}`} log={log} />)}
            </tbody>
          </table>
        )}
      </div>
      ...
      {pages > 1 && (
        <div className="p-4 border-t border-white/5 flex items-center justify-between">
          <span className="text-xs text-slate-500">Page {page} of {pages} ({total.toLocaleString()} total)</span>
          <div className="flex items-center gap-2">
            <button className="btn-ghost py-1.5 px-3 text-xs" onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1 || isFetching}>
            ...
            {Array.from({ length: Math.min(5, pages) }, (_, i) => {
              ...
              return (
                <button key={p} onClick={() => setPage(p)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${p === page ? 'bg-violet-500/20 text-violet-300 border border-violet-500/30' : 'text-slate-500 hover:text-slate-300'}`}>
                  {p}
                </button>
              );
            })}
            <button className="btn-ghost py-1.5 px-3 text-xs" onClick={() => setPage(p => Math.min(pages, p + 1))} disabled={page === pages || isFetching}>
