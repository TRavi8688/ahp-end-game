...
import { Calendar, Clock, UserCheck, RefreshCw, AlertCircle, Search, ArrowRight } from 'lucide-react';
...
export default function TodaysAppointmentsPage() {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      // Get all appointments (backend returns hospital appointments based on staff credentials)
      ...
      const allAppts = res.data.data.items || [];
      
      // Filter today's appointments in local timezone
      const todayStr = new Date().toDateString();
      const filtered = allAppts.filter((a: Appointment) => {
        return new Date(a.scheduled_at).toDateString() === todayStr;
      });
  ...
  const handleCheckIn = async (appt: Appointment) => {
    try {
      ...
      alert(`${appt.patient_name || 'Patient'} checked in successfully! They are now in the waiting queue.`);
    } catch (err: any) {
      alert(err.response?.data?.detail || 'Failed to check in appointment.');
    } finally {
  ...
  const filteredAppts = appointments.filter(appt => {
    ...
    return (
      ...
      (appt.patient_phone?.includes(query)) ||
  ...
  const afternoonList = filteredAppts.filter(a => {
    const hrs = new Date(a.scheduled_at).getHours();
    return hrs >= 12 && hrs < 17;
  });
    ...
    <div className="min-h-screen p-6 md:p-10 bg-[#050508] text-[#f8fafc] font-inter overflow-x-hidden">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
        <div className="space-y-2">
          ...
          <h1 className="text-4xl font-black tracking-tighter outfit leading-none text-white">Today's Appointments</h1>
      ...
      <div className="grid grid-cols-3 gap-6 mb-12">
        {[
          { label: 'Total Appts', value: appointments.length, color: 'text-blue-500', bg: 'from-blue-500/10 to-transparent border-blue-500/10' },
          { label: 'Pending Scheduled', value: appointments.filter(a => a.status === 'scheduled').length, color: 'text-amber-500', bg: 'from-amber-500/10 to-transparent border-amber-500/10' },
          { label: 'Checked In', value: appointments.filter(a => a.status === 'confirmed').length, color: 'text-emerald-500', bg: 'from-emerald-500/10 to-transparent border-emerald-500/10' },
        ].map((stat, i) => (
          <div key={i} className={`bg-gradient-to-br ${stat.bg} bg-white/[0.02] border border-white/5 rounded-[28px] p-6 relative overflow-hidden transition-all hover:scale-[1.01]`}>
            <h2 className={`text-4xl font-black outfit tracking-tighter ${stat.color}`}>{stat.value}</h2>
          ...
          {[
            ...
            return (
              <div key={title} className="space-y-4">
                <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest px-1">{title} &bull; {list.length}</h3>
                  ...
                  {list.map(appt => (
                    <div key={appt.id} className="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:bg-white/[0.01] transition-all">
                      ...
                      <div className="flex items-center gap-6">
                        <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex flex-col items-center justify-center font-bold text-xs outfit text-blue-400">
                        ...
                        <div>
                          <div className="flex items-center gap-3 mb-1">
                            <h4 className="text-lg font-bold text-white outfit tracking-tight leading-none">
                      ...
                      <div>
                        {appt.status === 'scheduled' ? (
                          <button
                            ...
                            className="bg-blue-600 hover:bg-blue-500 disabled:bg-blue-600/50 text-white px-6 py-3.5 rounded-xl font-bold text-[10px] tracking-widest uppercase transition-all flex items-center gap-2 shadow-lg shadow-blue-600/10"
                          >
