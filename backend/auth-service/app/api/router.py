...
async def register_user(
    ...
    if result.scalars().first():
        return error_response(
            "USER_EXISTS", "Email already registered", status_code=409
        )
    ...
    if user_in.phone_number:
        ...
        if phone_check.scalars().first():
            return error_response(
                "PHONE_EXISTS", "Phone number already registered", status_code=409
            )
...
async def login(
    ...
    if not user.is_active:
        ...
        return error_response(
            "INACTIVE_USER", "User account is inactive", status_code=403
        )
    ...
    log_audit_event(
        action="user_login_success", actor_id=str(user.id), target_id=str(user.id)
    )
...
async def refresh_access_token(
    ...
    if not decoded:
        return error_response(
            "INVALID_TOKEN", "Refresh token is invalid or expired", status_code=401
        )
...
async def verify_password_reset_otp(
    ...
    if not otp_record or otp_record.is_verified:
        return error_response(
            "INVALID_OTP", "OTP is invalid or already used", status_code=400
        )

    # Check expiry
    if otp_record.expires_at < datetime.now(timezone.utc):
        return error_response(
            "EXPIRED_OTP", "OTP has expired. Please request a new one.", status_code=400
        )
...
async def reset_password(
    ...
    if not matched_record:
        return error_response(
            "INVALID_TOKEN", "Reset token is invalid or expired", status_code=400
        )

    # Get the user
    user_result = await db.execute(
        select(User).where(User.id == matched_record.user_id)
    )
    user = user_result.scalars().first()
    if not user:
        return error_response("USER_NOT_FOUND", "User not found", status_code=404)
    ...
    log_audit_event(
        action="user_password_reset", actor_id=str(user.id), target_id=str(user.id)
    )

    logger.info(
        f"Password reset completed for user_id={user.id}, all tokens revoked (version={user.token_version})"
    )
...
async def change_password(
    ...
    if not verify_password(req.current_password, user.hashed_password):
        return error_response(
            "WRONG_PASSWORD", "Current password is incorrect", status_code=400
        )
    ...
    log_audit_event(
        action="user_password_changed", actor_id=str(user.id), target_id=str(user.id)
    )
    ...
    if jti and exp:
        ...
        if remaining > 0:
            try:
                await blacklist_token(jti, remaining)
                logger.info(
                    f"Token blacklisted on logout: jti={jti}, user={payload.get('sub')}"
                )
