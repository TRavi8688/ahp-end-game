...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def _get_client_ip(self, request: Request) -> str:
        ...
        if client_host in ("127.0.0.1", "::1", "localhost", "unknown"):
            is_trusted = True
        elif client_host.startswith("172.") or client_host.startswith("10.") or client_host.startswith("192.168."):
            is_trusted = True
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if request.url.path in ("/health", "/health/auth", "/health/healthcare", "/ready"):
            return await call_next(request)
