...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column('record_type', sa.String(length=50), nullable=True, server_default='document'),
        sa.Column('ai_summary', sa.Text(), nullable=True),
        sa.Column('ai_extracted', sa.Text(), nullable=True),
        sa.Column('raw_text', sa.Text(), nullable=True),
        sa.Column('patient_summary', sa.Text(), nullable=True),
        sa.Column('hidden_by_patient', sa.Boolean(), nullable=False, server_default=sa.text('false')),
    ...
    op.create_index('ix_medical_records_patient', 'medical_records', ['patient_id'], unique=False)
