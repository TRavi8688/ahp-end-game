...
class PaymentTransaction(Base):
    ...
    amount: Mapped[int] = mapped_column(Integer, nullable=False)  # Stored in paise/cents (integer)
    payment_method: Mapped[str] = mapped_column(String(20), nullable=False)  # cash, card, upi
    status: Mapped[str] = mapped_column(String(20), default="completed", nullable=False)
    transaction_reference: Mapped[str] = mapped_column(String(100), nullable=True, index=True)
