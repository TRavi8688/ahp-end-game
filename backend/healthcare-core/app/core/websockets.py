...
class ConnectionManager:
    ...
    async def connect(self, hospital_id: str, websocket: WebSocket):
        ...
        logger.info(
            f"WebSocket connected for hospital: {hospital_id}. Total connections: {len(self.active_connections[hospital_id])}"
        )
    ...
    def disconnect(self, hospital_id: str, websocket: WebSocket):
        if hospital_id in self.active_connections:
            if websocket in self.active_connections[hospital_id]:
                ...
                logger.info(
                    f"WebSocket disconnected for hospital: {hospital_id}. Remaining: {len(self.active_connections[hospital_id])}"
                )
    ...
    async def broadcast_to_hospital(
        self, hospital_id: str, event_type: str, data: dict
    ):
        """
