/**
 * hospyn-v2-web/src/components/ActivationWizard.jsx
 *
 * REBUILT per Hospin rebrand plan:
 *  - Brand renamed Hospyn → Hospin throughout
 *  - Logo image used in sidebar instead of generic Shield icon
 *  - PAYMENT REMOVED ENTIRELY — no QR, no UPI, no card flow.
 *    Onboarding endpoints for payment (/generate-razorpay-qr, /submit-upi-vpa,
 *    /submit-card-payment, /verify-card-otp) are never called from this file.
 *  - Document upload (Step 2 biometrics) removed — registration goes straight
 *    from Corporate Coordinates to Phone OTP.
 *  - After OTP verified, hospital status = pending_verification.
 *    Owner sees "Application Submitted" screen, NOT instant dashboard access.
 *  - Back button: Step 2 (OTP) → Step 1, fields stay editable, resubmitting
 *    does not throw 409 (backend updates existing pending hospital instead
...
const V = {
  hospitalName: v => {
    ...
    if (!/^[a-zA-Z\s\-\.']+$/.test(v.trim())) return 'Only letters, spaces, hyphens allowed — no numbers';
  ...
  staffCount: v => {
    ...
    if (isNaN(n) || !Number.isInteger(n)) return 'Must be a whole number (e.g. 50)';
    if (n < 1) return 'Minimum 1 staff member';
    if (n > 50000) return 'Maximum 50,000 staff members';
    return '';
  },
  email: v => {
    if (!v?.trim()) return 'Email is required';
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(v.trim())) return 'Enter a valid email (e.g. owner@hospital.com)';
    return '';
  },
  password: v => {
  ...
  nabh: v => {
    if (!v?.trim()) return 'NABH registration number is required';
    if (v.trim().length < 5) return 'Enter a valid NABH number (min 5 characters)';
    return '';
  },
  phone: v => {
    const d = v?.replace(/\D/g, '');
    if (!d?.length) return 'Phone number is required';
    if (!/^[6-9]\d{9}$/.test(d)) return 'Must be a valid 10-digit Indian mobile (starts 6-9)';
    return '';
  },
  address: v => {
    if (!v?.trim() || v.trim().length < 10) return 'Enter a full address (min 10 characters)';
...
export default function ActivationWizard({ isOpen, onClose, onActivationSuccess }) {
  const [step, setStep] = useState(1);          // 1 = Coordinates, 2 = OTP, 3 = Submitted
  ...
  // ── Step 1 → Submit registration → auto-dispatch OTP → go to Step 2 ────────
  const submitStep1 = async () => {
    ...
    try {
      const data = await post('/onboarding/register-enterprise-simple', {
        // NOTE: uses the simplified registration endpoint (no documents, no payment)
        hospital_id: hospitalId,  // if re-submitting after Back, updates existing record
        name: f.name,
  ...
  const steps = ['Corporate Coordinates', 'Phone Verification', 'Application Submitted'];

  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[2000] bg-slate-900/75 backdrop-blur-md overflow-y-auto flex items-start lg:items-center justify-center p-4 lg:p-8">
          <motion.div initial={{ scale: 0.97, y: 16 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.97, y: 16 }} className="bg-white rounded-3xl shadow-2xl border border-slate-100 w-full max-w-5xl flex flex-col lg:flex-row overflow-hidden my-4">

            {/* Rail */}
            <div className="bg-slate-50 lg:w-[36%] p-8 lg:p-10 flex flex-col border-b lg:border-b-0 lg:border-r border-slate-200">
              <div className="flex items-center gap-2.5 mb-7">
                <img src={logoImg} alt="Hospin" className="w-8 h-8 object-contain" />
                <span className="font-black text-slate-900 tracking-tight text-lg">Hospin</span>
              </div>
              <h3 className="text-xl font-black text-slate-900 leading-tight mb-1.5">Hospital Registration</h3>
              <p className="text-slate-400 text-xs leading-relaxed mb-8">
                Verified in minutes. Our team manually reviews every application before activation.
              </p>
                ...
                {steps.map((lbl, i) => {
                  ...
                  return (
                    <div key={s} className="flex gap-3 items-start">
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black shrink-0 transition-all ${step > s ? 'bg-emerald-100 text-emerald-600 border border-emerald-200' : step === s ? 'bg-violet-600 text-white shadow-md' : 'bg-white border border-slate-200 text-slate-400'}`}>
                        {step > s ? <Check size={13} strokeWidth={3}/> : s}
                      </div>
                      <p className={`text-[11px] font-bold pt-1 ${step === s ? 'text-violet-600' : step > s ? 'text-slate-400 line-through' : 'text-slate-500'}`}>{lbl}</p>
                    </div>
                  );
                })}
              </div>
              <div className="mt-8 pt-6 border-t border-slate-200 flex items-center justify-between text-slate-400 text-[10px] font-medium">
            ...
            <div className="flex-1 p-7 lg:p-10 bg-white relative flex flex-col">
              <button onClick={onClose} className="absolute top-5 right-5 p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-all"><X size={18}/></button>
                ...
                <AnimatePresence mode="wait">
                  ...
                  {step === 1 && (
                    <motion.div key="s1" initial={{ opacity:0, x:12 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-12 }} className="space-y-5">
                      <div><h4 className="text-xl font-black text-slate-900">Corporate Coordinates</h4><p className="text-slate-400 text-xs mt-1">Tell us about your hospital. Verification happens after submission.</p></div>
                      <div className="space-y-4 max-h-[440px] overflow-y-auto pr-1.5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <div className="relative">
                              <input type={showPw ? 'text' : 'password'} value={f.owner_password} onChange={e => set('owner_password')(e.target.value)} placeholder="Min 8, 1 uppercase, 1 number, 1 symbol"
                                className={`w-full border rounded-xl p-3.5 text-sm outline-none transition-all pr-9 ${f.owner_password && V.password(f.owner_password) ? 'bg-rose-50/60 border-rose-300' : f.owner_password && !V.password(f.owner_password) ? 'bg-emerald-50/60 border-emerald-300' : 'bg-slate-50 border-slate-200 focus:bg-white focus:border-violet-500'}`}/>
                              <button type="button" onClick={() => setShowPw(p => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400">{showPw ? <EyeOff size={15}/> : <Eye size={15}/>}</button>
                            </div>
                        ...
                        <div className="grid grid-cols-5 gap-2 items-end">
                          ...
                          <div className="col-span-2">
                            <label className="block text-slate-700 text-[11px] font-semibold mb-1.5">Longitude</label>
                  ...
                  {/* STEP 3 — Application Submitted (pending_verification) */}
                  {step === 3 && (
                    <motion.div key="s3" initial={{ opacity:0, scale:0.96 }} animate={{ opacity:1, scale:1 }} className="text-center space-y-6 py-8">
                      ...
                      <button type="button" onClick={onClose} className="w-full max-w-sm mx-auto py-4 bg-slate-900 hover:bg-slate-800 text-white font-bold text-sm rounded-2xl transition-all shadow-lg block">
                        Close
                      </button>
                    </motion.div>
                  )}
