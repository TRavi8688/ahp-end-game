                            ...
                            {(data?.weekly_breakdown || [14000, 18000, 9500, 22000, 16000, 19500, 24000]).map((val, i) => {
                                ...
                                return (
                                    <Box key={i} sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                                        ...
                                        <Typography variant="caption" sx={{ color: '#475569', fontWeight: 700, fontSize: '0.65rem' }}>{days[i]}</Typography>
                                    </Box>
                                );
                            })}
                        </Box>
                    ...
                    <Card elevation={0} sx={{ bgcolor: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '24px', overflow: 'hidden' }}>
                                ...
                                <TableBody>
                                    {(data?.transactions || []).map((txn) => (
                                        <TableRow key={txn.id} sx={{ '& .MuiTableCell-root': { borderColor: 'rgba(255,255,255,0.04)', color: '#fff', py: 2.5 }, '&:hover': { bgcolor: 'rgba(255,255,255,0.02)' } }}>
                                            <TableCell>
                                                <Typography variant="caption" sx={{ fontFamily: 'Space Mono', color: '#475569', fontWeight: 700 }}>{txn.id}</Typography>
                                            </TableCell>
                                            <TableCell>
                                                <Typography variant="body2" sx={{ fontWeight: 700, color: '#fff' }}>{txn.patient_name}</Typography>
                                            </TableCell>
                                            <TableCell>
                                                <Typography variant="body2" sx={{ color: '#64748b' }}>{new Date(txn.date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</Typography>
                                            </TableCell>
                                            <TableCell>
                        ...
                        {(!data?.transactions || data.transactions.length === 0) && (
                            <Box sx={{ p: 6, textAlign: 'center' }}>
                                <Typography variant="body2" sx={{ color: '#64748b' }}>No transactions found for this period.</Typography>
                            </Box>
                        )}
                    </Card>
                </>
