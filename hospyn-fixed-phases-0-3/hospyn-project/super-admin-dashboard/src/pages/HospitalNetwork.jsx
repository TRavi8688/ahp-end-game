...
import {
  Building2, Search, Filter, ChevronRight, MapPin, Users, Clock,
  CheckCircle2, AlertTriangle, XCircle, Eye, RefreshCw, Loader2,
  TrendingUp, Activity, DollarSign, Shield
} from 'lucide-react';
...
export default function HospitalNetwork({ onViewHospital }) {
  ...
  const fetchHospitals = async () => {
    ...
    try {
      ...
      const [hospRes, analyticsRes] = await Promise.all([
        axios.get(`${API_BASE}/admin/hospitals`, cfg),
        axios.get(`${API_BASE}/analytics/overview`, cfg)
      ]);
      const all = hospRes.data?.data || [];
      setHospitals(all);
    ...
    <div className="h-full flex flex-col animate-fadeIn">
      ...
      <div className="p-6 border-b border-white/5">
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            { label: 'Total Registered', value: stats.total, color: 'indigo', cls: 'badge-blue' },
            { label: 'Verified & Active', value: stats.verified, color: 'emerald', cls: 'badge-green' },
            { label: 'Pending Review', value: stats.pending, color: 'amber', cls: 'badge-amber' },
            { label: 'Suspended', value: stats.suspended, color: 'rose', cls: 'badge-red' },
          ].map((s, i) => (
...
// Need Globe2 fallback
function Globe2({ size, className }) {
