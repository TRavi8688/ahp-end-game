...
The `fixes-merge` branch aggregates vital stability, security, and integration fixes across both backend and frontend layers:

### 1. Authentication Service & Security Hardening
* **Login Token Payload**: Updated `/api/v1/auth/login` to return `refresh_token` in addition to the `access_token` for proper session persistence on frontends and test compatibility.
* **Input Validation**: Added robust validations to the `/api/v1/auth/register` registration flow:
  * Restricts creation of weak passwords (forces a minimum length of 8 characters).
  * Validates email address syntax format.
* **Test Corrections**: Fixed the test suite expectations in `tests/test_auth.py` to match endpoint behaviors:
  * OTP verification failure returns a `400 Bad Request` instead of `401 Unauthorized`.
  * OTP generation accepted phone request returns `202 Accepted` format.

### 2. Frontend & Dashboard Updates
* **Super Admin Portal & Staff Portal**:
  * Cleaned up conflicting legacy assets and duplicate files.
  * Resolved standard layouts, routing flows, and ProtectedRoute guards for nurse dashboard, admin settings, and billing options.

### 3. Healthcare Core Integration
* Integrated billing, doctor schedule APIs, staff and owner dashboards, patient vitals logs, and ticket modules.
* Cleaned up migration files, supporting both local Postgres instances and production database schemas.
