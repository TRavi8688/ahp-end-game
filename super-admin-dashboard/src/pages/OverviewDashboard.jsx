// super-admin-dashboard/src/pages/OverviewDashboard.jsx
// FIXED:
//   1. Removed all localStorage.getItem('token') calls — uses api from lib/apiClient
//   2. onNavigate now uses useNavigate() hook — no prop needed from App.jsx
...
import {
  ...
  ArrowUpRight, Zap, Globe, RefreshCw, Terminal,
  ChevronRight, Server, Database, DollarSign
} from 'lucide-react';
...
const StatCard = ({ icon, label, value, sub, color, loading, trend }) => (
  <div className={`glass-card p-5 stat-card-glow-${color} animate-fadeIn`}>
...
export default function OverviewDashboard() {
  ...
  const fetchData = async () => {
    try {
      const [analyticsData, auditData] = await Promise.all([
        ...
        api.get('/api/v1/admin/audit-logs?limit=15'),
      ]);
      setMetrics(analyticsData?.metrics || analyticsData);
      const logs = Array.isArray(analyticsData?.recent_audit_events)
        ? analyticsData.recent_audit_events
        : (Array.isArray(auditData) ? auditData : auditData?.logs || []);
      setAuditLogs(logs);
      setLastRefresh(new Date());
    } catch (e) {
      console.error('OverviewDashboard fetch error:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const onNavigate = (view) => navigate(quickNavMap[view] || '/');
  ...
  return (
    <div className="h-full overflow-y-auto p-6 space-y-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={<Building2 />} label="Verified Hospitals"   value={(metrics?.active_hospitals || 0).toLocaleString()}     sub={`${metrics?.pending_verifications || 0} pending`} color="indigo"  loading={loading} trend={8}  />
        <StatCard icon={<Users />}     label="Global Patients"      value={(metrics?.registered_patients || 0).toLocaleString()}   sub="Across all nodes"                                 color="emerald" loading={loading} trend={12} />
        <StatCard icon={<ShieldCheck />} label="Active Staff"       value={(metrics?.registered_staff || 0).toLocaleString()}      sub="Doctors · Nurses · Admins"                        color="amber"   loading={loading} />
        <StatCard icon={<Activity />}  label="System Health"        value="Optimal"                                                 sub="99.98% uptime"                                    color="cyan"    loading={loading} />
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={i} className="glass-card p-4 flex items-center gap-4">
            <div className={`p-2.5 rounded-lg bg-${node.color}-500/10 text-${node.color}-400`}>{node.icon}</div>
