...
class Doctor(Base):
    ...
    ahba_id: Mapped[Optional[str]] = mapped_column(String(100), unique=True, index=True, nullable=True)
    certification_url: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)
...
class BreakTypeEnum(str, enum.Enum):
    bio = "Bio Break"
    lunch = "Lunch Break"
    meeting = "Meeting"
    training = "Training"
...
class DoctorSession(Base):
    ...
    total_treatment_minutes: Mapped[int] = mapped_column(Integer, default=0)
    
    breaks: Mapped[List["DoctorBreak"]] = relationship(back_populates="session", cascade="all, delete-orphan")

class DoctorBreak(Base):
    __tablename__ = "doctor_breaks"
    
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4, index=True)
    session_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("doctor_sessions.id"), index=True)
    break_type: Mapped[BreakTypeEnum] = mapped_column(SQLEnum(BreakTypeEnum), default=BreakTypeEnum.bio)
    start_time: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    end_time: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    
    session: Mapped["DoctorSession"] = relationship(back_populates="breaks")

class PatientTreatmentLog(Base):
    __tablename__ = "patient_treatment_logs"
    
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4, index=True)
    doctor_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("doctors.id"), index=True)
    patient_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("patients.id"), index=True)
    session_id: Mapped[Optional[uuid.UUID]] = mapped_column(ForeignKey("doctor_sessions.id"), index=True, nullable=True)
    
    start_time: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    end_time: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    treatment_minutes: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
