...
async def get_patient_appointments(
    ...
    """
    Returns upcoming and past appointments (PatientVisits) for the patient.
    """
    # Fetch visits
    ...
    for v in visits:
        ...
        h_name = (await db.execute(h_stmt)).scalar() or "Hospyn Clinic"
        
        apt_data = {
            "id": str(v.id),
            "doctor": v.doctor_name or "Assigned Doctor",
            "specialty": v.department or "General Consultation",
            "date": v.check_in_time.strftime("%d %b %Y") if v.check_in_time else "TBD",
            "time": v.check_in_time.strftime("%I:%M %p") if v.check_in_time else "TBD",
            "hospital_name": h_name,
            "status": v.status.value if hasattr(v.status, 'value') else v.status
        }
        
        if v.status in ["scheduled", "active"]:
...
async def get_patient_vitals(
    ...
    """
    Returns the latest vitals for the patient.
    """
    ...
    stmt = select(models.MedicalRecord).where(
        ...
        models.MedicalRecord.type == "vitals"
    ).order_by(models.MedicalRecord.created_at.desc())
    
    result = await db.execute(stmt)
    vitals_record = result.scalars().first()
    
    if not vitals_record or not vitals_record.ai_extracted:
        # Return fallback dummy structure if no vitals exist
        return {
            "bloodPressure": { "value": '120 / 80', "status": '✓ NORMAL', "statusColor": 'positive' },
            "heartRate": { "value": '72', "status": '✓ NORMAL', "statusColor": 'positive' },
            "bloodOxygen": { "value": '98%', "percent": 98, "status": '✓ NORMAL', "statusColor": 'positive' },
            "temperature": { "value": '98.6°F', "status": '✓ NORMAL', "statusColor": 'positive' },
            "lastUpdated": "NO DATA"
        }
        
    data = vitals_record.ai_extracted
    return {
        "bloodPressure": data.get("bloodPressure", { "value": '128 / 84', "status": '↑ ELEVATED', "statusColor": 'warning' }),
        "heartRate": data.get("heartRate", { "value": '76', "status": '✓ NORMAL', "statusColor": 'positive' }),
        "bloodOxygen": data.get("bloodOxygen", { "value": '96%', "percent": 96, "status": '✓ NORMAL', "statusColor": 'positive' }),
        "temperature": data.get("temperature", { "value": '98.4°F', "status": '✓ NORMAL', "statusColor": 'positive' }),
        "lastUpdated": vitals_record.created_at.strftime("%I:%M %p")
    }
...
async def get_my_records(
    ...
    # AI_HIDDEN: Exclude records auto-generated by Chitti AI from the patient-facing vault.
    # These records are saved to the DB for audit purposes but are never shown to the patient.
    CHITTI_AI_FACILITY_NAMES = ['Chitti AI Companion', 'Chitti AI', 'chitti_ai']
    result = await db.execute(
        select(models.MedicalRecord).where(
            models.MedicalRecord.patient_id == current_patient.id,
            models.MedicalRecord.family_member_id == active_member_id,
            models.MedicalRecord.hidden_by_patient == False,
            ~models.MedicalRecord.hospital_name.in_(CHITTI_AI_FACILITY_NAMES)
        )
    )
    await log_audit_action(
        db, 
...
async def get_patient_profile(
    ...
    try:
        ...
        # Prefer user-level hospyn_id if it has been directly updated in the DB,
        # fallback to patient-level hospyn_id. This ensures manual DB edits to users.hospyn_id
        # (e.g. for todendalaharshitha2004@gmail.com) are immediately reflected.
        effective_hospyn_id = (user.hospyn_id if user and user.hospyn_id else None) or patient.hospyn_id
