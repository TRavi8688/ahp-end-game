...
class ForensicTelemetryMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # 3. Proceed with Request (contextualized for lightweight tracing)
        from app.core.logging import logger
        with logger.contextualize(trace_id=trace_id, tenant_id=tenant_id, user_id=user_id, request_id=trace_id):
            ...
            if request.method != "GET" or response.status_code >= 400:
                async def record_audit_in_background():
                    ...
                    try:
                        ...
                        logger.error(f"FORENSIC_TELEMETRY_AUDIT_LOG_FAILED: {audit_err}")

                import asyncio
                asyncio.create_task(record_audit_in_background())
